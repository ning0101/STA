...
public class Observation {
    ...
    public Observation(LocalDateTime formattedDateTime, String result) throws ParseException{
    	this.phenomenonTime = formattedDateTime.toEpochSecond(ZoneOffset.UTC);
        this.phenomenonTimeString = formattedDateTime.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
        this.result_String = result;
        if(result.equalsIgnoreCase("-998.00")) {
        	this.result_String = "0.00";
        }
    }
    public Observation(String phenomenonTimeString, double result) throws ParseException{
    	...
    	LocalDateTime date = LocalDateTime.parse(test);
    	//LocalDate date = LocalDate.parse(test);
    	LocalDateTime datetime = LocalDateTime.parse(date.toString(),DateTimeFormatter.ISO_LOCAL_DATE_TIME);
    ...
    public Observation(String phenomenonTimeString, int result) throws ParseException{
    	LocalDate date = LocalDate.parse(phenomenonTimeString);
    	LocalDateTime datetime = LocalDateTime.parse(date.atTime(8, 0, 0, 0).toString(),DateTimeFormatter.ISO_LOCAL_DATE_TIME);
        this.phenomenonTime = datetime.toEpochSecond(ZoneOffset.UTC);
        this.phenomenonTimeString = datetime.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
